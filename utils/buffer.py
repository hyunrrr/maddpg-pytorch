...
class ReplayBuffer(object):
    ...
    def push(self, observations, actions, rewards, next_observations, dones):
        ...
        for agent_i in range(self.num_agents):
            ...
            ###
            #print('buffer_obs.type: ', type(observations))
            #print('buffer_obs.shape: ', observations.shape)
            #print('buffer_actions: ', actions)
            #print('buffer_vstack(observations[:, agent_i]).shape: ', np.vstack(observations[:, agent_i]).shape)
            #print('buffer_rewards: ', rewards)
            ...
            ###
            #print('buffer_dones.type: ', type(dones))
            #print('buffer_dones: ', dones)
            #print('buffer_dones[0]; ', dones[0])
            #print('buffer_agent_i: ', agent_i)
            #print('buffer_dones[:, agent_i]: ', dones[:, agent_i])   
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        ###
        #print('buffer_sample_inds: ', inds)
        #print('buffer_sample_inds.shape: ', inds.shape)
        #print('buffer_part_size: ', getsizeof(self.obs_buffs[0]))
        #print('buffer_part_type: ', self.obs_buffs[0].dtype)
